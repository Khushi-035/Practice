...
int ask(int start_node, const std::vector<int>& nodes) {
    std::cout << "? " << start_node << " " << nodes.size();
    for (int node : nodes) {
        std::cout << " " << node;
    }
    std::cout << std::endl; // std::endl flushes the output
...
void solve() {
    ...
    std::map<int, std::vector<int>> nodes_by_dist;
    std::vector<int> all_nodes(n);
    std::iota(all_nodes.begin(), all_nodes.end(), 1);

    for (int i = 1; i <= n; ++i) {
        d[i] = ask(i, all_nodes);
        nodes_by_dist[d[i]].push_back(i);
    }

    // Find the starting node of the longest path
    int max_len = 0;
    int start_node = -1;
    for (int i = 1; i <= n; ++i) {
        if (d[i] > max_len) {
    ...
    while (path.size() < (size_t)max_len) {
        int len_needed = d[current_node] - 1;
        
        std::vector<int> candidates;
        if (nodes_by_dist.count(len_needed)) {
            for (int node : nodes_by_dist[len_needed]) {
                if (!in_path[node]) {
        ...
        if (candidates.size() == 1) {
            ...
            // Elimination process to find the correct next node
            for (size_t i = 0; i < candidates.size(); ++i) {
                int node_to_test = candidates[i];
                
                std::vector<int> query_nodes = path;
                for (int c : candidates) {
                    if (c != node_to_test) {
                        query_nodes.push_back(c);
                    }
                }
                
                // If removing the test node shortens the path, it's the one we need
                if (ask(current_node, query_nodes) < d[current_node]) {
                    next_node = node_to_test;
                    break;
                }
            }
            // In the case where all but one candidate are eliminated, the last one is the answer
             if (next_node == -1 && !candidates.empty()) {
