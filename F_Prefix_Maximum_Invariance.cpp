...
void solve() {
    ...
    int i = 1;
    while (i <= n) { cin >> a[i]; vmax = max(vmax, a[i]); ++i; }
    i = 1;
    while (i <= n) { cin >> b[i]; vmax = max(vmax, b[i]); ++i; }

    const int V = max(vmax, 2 * n);
    auto rev = [&](int val) -> int { return V - val + 1; };
    ...
    i = 1;
    while (i <= n) {
        int pos_ge = fw.query(rev(a[i]));
        fw.update(rev(a[i]), i);
        int last_ge_y = fw.query(rev(b[i]));

        long long aCount = 1LL * (i - pos_ge) * (b[i] == a[i]);
        long long bCount = 1LL * min(pos_ge, last_ge_y);
        long long contrib = aCount + bCount;
